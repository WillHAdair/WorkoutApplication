...
class ScheduleData extends ChangeNotifier {
  ...
  void addSchedule(String scheduleName) {
    Schedule newSchedule =
        Schedule(name: scheduleName, period: 0, workouts: []);
  ...
  void addWorkoutsToDay(DateTime day, List<Workout> workouts) {
    db.addWorkoutsToDay(convertDateTimeToString(day),
        workouts.map((workout) => workout.name).toList());
    notifyListeners();
  }
  ...
  void editSchedule(
      String name, int period, List<Workout> workouts, bool? isCCurrent) {
